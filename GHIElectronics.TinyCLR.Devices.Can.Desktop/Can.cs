...
// Public surface mirrors GHIElectronics.TinyCLR.Devices.Can\Can.cs.
// Bodies on Desktop are safe no-ops; ReadMessages returns 0 and
// MessageReceived/ErrorReceived events never fire.
namespace GHIElectronics.TinyCLR.Devices.Can {
    public class CanController : IDisposable {
        ...
        public static CanController GetDefault() => CanController.FromName("Simulator");
        ...
        public event MessageReceivedEventHandler MessageReceived {
            add => this.messageReceivedCallbacks += value;
            remove => this.messageReceivedCallbacks -= value;
        }
    ...
    public class Filter {
        public enum IdType { Standard = 0, Extended = 1 }
        public enum FilterType { Range = 0, Mask = 1 }
    ...
    public enum CanError { ReadBufferOverrun = 0, ReadBufferFull = 1, BusOff = 2, Passive = 3 }
    public enum ErrorStateIndicator { Active = 0, Passive = 1 }
    ...
    namespace Provider {
        ...
        public sealed class CanControllerApiWrapper : ICanControllerProvider {
            ...
            public int MessagesToWrite => 0;
            public int MessagesToRead => 0;
            public bool CanWriteMessage => true;
            public bool CanReadMessage => false;
            public int WriteErrorCount => 0;
            public int ReadErrorCount => 0;
            public int SourceClock => 100_000_000;

            public event MessageReceivedEventHandler MessageReceived { add { } remove { } }
            public event ErrorReceivedEventHandler ErrorReceived { add { } remove { } }

            public CanControllerApiWrapper(NativeApi api) => this.Api = api;

            public void Dispose() { }
            public void Enable() { }
            public void Disable() { }
            public int WriteMessages(CanMessage[] messages, int offset, int count) => count;
            public int ReadMessages(CanMessage[] messages, int offset, int count) => 0;
            public void SetNominalBitTiming(CanBitTiming bitTiming) { }
            public void SetDataBitTiming(CanBitTiming bitTiming) { }
            public void AddFilter(Filter.IdType idType, Filter.FilterType filterType, uint id1, uint id2) { }
            public void RejectRemoteFrame(Filter.IdType idType) { }
