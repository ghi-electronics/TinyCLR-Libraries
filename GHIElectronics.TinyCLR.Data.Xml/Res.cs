...
namespace System.Xml
{
    ...
    internal class Res
    {
        ...
        public static string GetString(int res) => Xml_Errors[res];

        public static string GetString(int res, object arg) => Format(Xml_Errors[res], arg);

        public static string GetString(int res, object[] args) => Format(Xml_Errors[res], args);

        public static string GetString(int res, string arg) => Format(Xml_Errors[res], arg);

        public static string GetString(int res, string arg1, string arg2) => Format(Xml_Errors[res], new string[] { arg1, arg2 });

        private static string Format(string format, object arg0) => Format(format, 0, arg0);
        ...
        {
            var argSpecifier = "{" + argIndex + "}";
            var index = format.IndexOf(argSpecifier);

            if (index == -1) //argSpecifier not found, so nothing to replace
                return format;

            var result = format.Substring(0, index);
