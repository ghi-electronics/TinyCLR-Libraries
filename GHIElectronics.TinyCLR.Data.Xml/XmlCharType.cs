...
namespace System.Xml
{
    ...
    internal struct XmlCharType
    {
        ...
        static void InitInstance()
        {
            ...
            var chProps = new byte[CharPropertiesSize];
        ...
        {
            var rngs = ranges.ToCharArray();
            for (var p = 0; p < rngs.Length; p += 2)
            {
        ...
        public bool IsWhiteSpace(char ch)
        {
            ...
            if (ch < CharPropertiesSize)
                return (this.charProperties[ch] & fWhitespace) != 0;
            ret = searchRangeString(ch, s_Whitespace);
#if DEBUG
            var check = linearSearchRangeString(ch, s_Whitespace);
        ...
        public bool IsLetter(char ch)
        {
            ...
            bool ret;
            if (ch < CharPropertiesSize)
                return (this.charProperties[ch] & fLetter) != 0;
            ret = searchRangeString(ch, s_Letter);
#if DEBUG
            var check = linearSearchRangeString(ch, s_Letter);
            Debug.Assert(ret == check, "no match for char " + ch);
#endif
            return ret;
#else
        ...
        public bool IsExtender(char ch) => (ch == 0xb7);
        ...
        public bool IsNCNameChar(char ch)
        {
            ...
            bool ret;
            if (ch < CharPropertiesSize)
                return (this.charProperties[ch] & fNCName) != 0;
            ret = searchRangeString(ch, s_NCName);
#if DEBUG
            var check = linearSearchRangeString(ch, s_NCName);
            Debug.Assert(ret == check, "no match for char " + ch);
#endif
            return ret;
#else
        ...
        public bool IsStartNCNameChar(char ch)
        {
            ...
            bool ret;
            if (ch < CharPropertiesSize)
                return (this.charProperties[ch] & fNCStartName) != 0;
            ret = searchRangeString(ch, s_NCStartName);
#if DEBUG
            var check = linearSearchRangeString(ch, s_NCStartName);
            Debug.Assert(ret == check, "no match for char " + ch);
#endif
            return ret;
#else
        ...
        public bool IsCharData(char ch)
        {
            ...
            bool ret;
            if (ch < CharPropertiesSize)
                return (this.charProperties[ch] & fCharData) != 0;
            ret = searchRangeString(ch, s_CharData);
#if DEBUG
            var check = linearSearchRangeString(ch, s_CharData);
            Debug.Assert(ret == check, "no match for char " + ch);
#endif
            return ret;
#else
        ...
        public bool IsPubidChar(char ch)
        {
            ...
            bool ret;
            if (ch < CharPropertiesSize)
                return (this.charProperties[ch] & fPublicId) != 0;
            ret = searchRangeString(ch, s_PublicID);
#if DEBUG
            var check = linearSearchRangeString(ch, s_PublicID);
            Debug.Assert(ret == check, "no match for char " + ch);
#endif
            return ret;
#else
...
#if DELAY_LOOKUP
            if (ch < CharPropertiesSize)
                return (this.charProperties[ch] & fText) != 0;
...
#if DELAY_LOOKUP
            if (ch < CharPropertiesSize)
                return (this.charProperties[ch] & fAttrValue) != 0;
            return searchRangeString(ch, s_CharData);
#else
        ...
        public bool IsNameChar(char ch) => this.IsNCNameChar(ch) || ch == ':';

        public bool IsStartNameChar(char ch) => this.IsStartNCNameChar(ch) || ch == ':';

        public bool IsDigit(char ch) => (ch >= 0x30 && ch <= 0x39);

        public bool IsHexDigit(char ch) => (ch >= 0x30 && ch <= 0x39) || (ch >= 'a' && ch <= 'f') || (ch >= 'A' && ch <= 'F');

        internal bool IsOnlyWhitespace(string str) => this.IsOnlyWhitespaceWithPos(str) == -1;
        ...
        internal int IsOnlyWhitespaceWithPos(string str)
        {
            if (str != null)
            {
                ...
                {
                    if ((this.charProperties[str[i]] & fWhitespace) == 0)
                    {
                        return i;
                    }
                }
        ...
        internal bool IsNmToken(string str)
        {
            ...
            for (var i = 0; i < str.Length; i++)
            {
                if ((this.charProperties[str[i]] & fNCName) == 0 && str[i] != ':')
                {
                    return false;
                }
            }
        ...
        internal int IsOnlyCharData(string str)
        {
            ...
            if (str != null)
            {
                for (var i = 0; i < str.Length; i++)
                {
                    if ((this.charProperties[str[i]] & fCharData) == 0)
                    {
                        return i;
                    }
                }
        ...
        internal int IsPublicId(string str)
        {
            if (str != null)
            {
                for (var i = 0; i < str.Length; i++)
                {
                    if ((this.charProperties[str[i]] & fPublicId) == 0)
                    {
                        return i;
                    }
                }
