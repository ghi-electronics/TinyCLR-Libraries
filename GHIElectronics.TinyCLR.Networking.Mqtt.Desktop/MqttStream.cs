...
namespace GHIElectronics.TinyCLR.Networking.Mqtt {
    internal class MqttStream {
        ...
        public bool DataAvailable {
            get {
                if (this.sslProtocol != SslProtocols.None)
                    throw new NotSupportedException("TODO - Not supported on Desktop: TinyCLR-style SslStream(Socket).");
        ...
        public void Connect() {
            ...
            if (this.sslProtocol != SslProtocols.None) {
                throw new NotSupportedException("TODO - Not supported on Desktop: TinyCLR-style SslStream(Socket) ctor + AuthenticateAsClient(host, caCert, clientCert, sslProtocol).");
            }
        }

        public int Send(byte[] buffer) {
            if (this.sslProtocol != SslProtocols.None)
                throw new NotSupportedException("TODO - Not supported on Desktop: SSL.");
