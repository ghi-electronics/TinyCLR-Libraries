...
namespace GHIElectronics.TinyCLR.Cryptography {
    // CRC16-CCITT (polynomial 0x1021, init 0x0000). Pure managed equivalent of
    // TinyCLR's native firmware Crc16 — produces identical output on Desktop + Device.
    public class Crc16 {
        ...
        public ushort ComputeHash(byte[] data, int offset, int count) => this.ComputeHash(data, offset, count, 0);
        ...
        public ushort ComputeHash(byte[] data, int offset, int count, ushort seed) {
            ...
            for (var i = 0; i < count; i++)
                crc = (ushort)((crc << 8) ^ Crc16Tab[((crc >> 8) ^ data[offset + i]) & 0x00FF]);
            return crc;
        }
    }
}
