...
namespace GHIElectronics.TinyCLR.UI.Media {
    ...
    public abstract class Brush {
        ...
        /// <summary>Fills a rectangle with rounded corners (radius in pixels). Cheap: only the corner pixels
        /// rasterize. Brushes that don't override it fall back to a square fill (radius ignored).</summary>
        internal virtual void RenderRectangle(Bitmap bmp, Pen outline, int x, int y, int width, int height, int xCornerRadius, int yCornerRadius) =>
            this.RenderRectangle(bmp, outline, x, y, width, height);
