...
namespace GHIElectronics.TinyCLR.UI.Controls {
    ...
    public class DataGrid : ContentControl {
        ...
        protected virtual void Dispose(bool disposing) {
            ...
            if (disposing) {
                // Managed cleanup; finalizer path skips this. Font is owned by
                // the caller (passed via constructor), so we don't touch it.
        ...
        public override void OnRender(DrawingContext dc) {
            // Bail before any of the inner allocations / divisions if the grid
            // was constructed degenerate (width=0, rowCount=0, rowHeight=0).
            // Otherwise we either throw out of OnRender or produce 0-byte
            // bitmaps that fail downstream.
            if (this.ActualWidth <= 0 || this.ActualHeight <= 0) return;
            if (this.RowHeight <= 0 || this.rowCount <= 0) return;
